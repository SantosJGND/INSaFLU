...
from managing_files.models import DataSet, MixedInfectionsTag, Reference
...
class Command(BaseCommand):
        ...
        if not uploadFile is None:
            try:
                if not software.is_exist_database_abricate(uploadFile.abricate_name):
                    self.stdout.write(
                        "Create new Abricate database: {}".format(
    ...
    def test_gff_and_bed_files(self):
        ...
        for reference in Reference.objects.all():
            if reference.is_deleted:
                continue
            if "teleflu" in reference.reference_fasta_name:
                continue
            count += 1
            ...
            if not os.path.exists(
                ...
                self.stdout.write(
                    "Reference removed: '{}' owner: '{}'".format(
                        reference.name, reference.owner.username
                    )
                )
    ...
    def upload_default_references(self):
        ...
        try:
            ...
            self.stdout.write(
                "Upload References failed because there's not default user.\n"
        ...
        uploadFiles = UploadFiles()
        b_test = False
        uploadFiles.upload_default_references(
